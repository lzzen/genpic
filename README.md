...
`go run ./cmd/genpic` 提供 **`POST /api/generate`**（与嵌入式主页一致）：请求 JSON 必须包含 **`base_url`**、**`api_key`**，由服务端转发到第三方；**无需**事先 `export GEMINI_*` 等环境变量即可在网页里生图。全平台始终启用任务存储：该接口返回 **`202 Accepted`** 与任务对象，客户端需轮询 **`GET /v1/jobs/{id}`** 直至成功或失败；终端 **stderr** 仍会打印上游请求/响应（超长 **base64**、**thoughtSignature** 会脱敏）。
...
|---|---|
| `POST /api/generate` | Enqueue generation (`202` + job); poll `GET /v1/jobs/{id}` (embedded UI does this automatically) |
| `GET  /v1/models` | List available models |
| `GET  /v1/jobs/{id}` | Job status and image `data` when succeeded |
| `GET  /v1/jobs` | List recent jobs for the caller's session / user headers |
...
**`POST {address}/api/generate`** (not under `/v1`). **`GET {address}/v1/models`**
and **`GET {address}/v1/jobs/...`** remain useful for discovery and polling.
...
| M1 — async `POST /api/generate` + jobs (`/v1/jobs`); DB + object storage | Partial (async + poll) |
