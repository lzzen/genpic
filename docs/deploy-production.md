...
**Full Platform（`cmd/genpic`）**：**`mvp_lite.default_base_url`** 与 **`GET /api/public-config`** 同上；监听端口用 **`server.port`**（**`PORT`** 优先）。`systemd` 的 `ExecStart` 改为 **`…/genpic -config …/config.yaml`** 即可。本文仍以 **MVP Lite** 二进制名为例。
